...
@protocol WinkProgressDelegate;

typedef enum {
    /** Progress is shown using an UIActivityIndicatorView. This is the default. */
    WinkProgressModeIndeterminate,
    /** Progress is shown using a WinkProgressView. */
	WinkProgressModeDeterminate,
	/** Shows a custom view */
	WinkProgressModeCustomView
} WinkProgressMode;

typedef enum {
    /** Opacity animation */
    WinkProgressAnimationFade,
    /** Opacity + scale animation */
    WinkProgressAnimationZoom
} WinkProgressAnimation;


@interface WinkProgress : UIView {
	
	WinkProgressMode mode;
    WinkProgressAnimation animationType;
	...
	id<WinkProgressDelegate> delegate;
	NSString *labelText;
...
/** 
 * WinkProgress operation mode. Switches between indeterminate (MBProgressHUDModeIndeterminate) and determinate
 * progress (MBProgressHUDModeDeterminate). The default is MBProgressHUDModeIndeterminate.
 *
 * @see WinkProgressMode
 */
@property (assign) WinkProgressMode mode;

/**
 * The animation type that should be used when the HUD is shown and hidden. 
 *
 * @see WinkProgressAnimation
 */
@property (assign) WinkProgressAnimation animationType;

/** 
 * The HUD delegate object. If set the delegate will receive hudWasHidden callbacks when the HUD was hidden. The
 * delegate should conform to the WinkProgressDelegate protocol and implement the hudWasHidden method. The delegate
 * object will not be retained.
 */
@property (assign) id<WinkProgressDelegate> delegate;
...
@protocol WinkProgressDelegate <NSObject>

@required

-(void)cancelProgressProcess:(id) sender;
 ...
 */
- (void)hudWasHidden:(WinkProgress *)hud;
 ...
 */
@interface WinkRoundProgressView : UIView {
@private
    float _progress;
}
