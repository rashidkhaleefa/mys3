...
//  WinkFramework
//
//  Created by rashid khaleefa in Labs on 30/04/12.
//  Copyright (c) 2012 CSS Corp  P Ltd. All rights reserved.
//


#import "WinkProgress.h"

@interface WinkProgress ()
...
- (void)setMode:(WinkProgressMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == WinkProgressModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(WinkRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == WinkProgressModeDeterminate) {
        self.indicator = [[[WinkRoundProgressView alloc] init] autorelease];
    }
    else if (mode == WinkProgressModeCustomView && self.customView != nil){
...
+ (WinkProgress *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    
	WinkProgress *hud = [[WinkProgress alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
	return hud ;//[hud autorelease];
}

+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	UIView *viewToRemove = nil;
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[WinkProgress class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		WinkProgress *HUD = (WinkProgress *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"MBProgressHUDViewIsNillException" 
					format:@"The view used in the WinkProgress initializer is nil."];
	}
...
{
    NSLog(@"cancel button touched up");
    if ([delegate respondsToSelector:@selector(cancelProgressProcess:)]) {
        [delegate performSelector:@selector(cancelProgressProcess:) withObject:self];
    }
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = WinkProgressAnimationFade;
        self.mode = WinkProgressModeIndeterminate;
        self.labelText = nil;
        ...
        // Add cancel button
        self.btnCancel = [[UIButton alloc] initWithFrame:self.bounds];
        [self.btnCancel addTarget:self action:@selector(modalViewAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        // Add details label delatils text was set
        //if (nil != self.btnCancel) 
//        {
//
//            float lHeight = dims.height + PADDING;
//            float lWidth;
//            if (dims.width <= (frame.size.width - 2 * margin)) {
//                lWidth = dims.width;
//            }
...
//			
//            // Set label position and dimensions
//            CGRect lFrameB = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset,
//                                        lFrame.origin.y + lFrame.size.height + PADDING*4, lWidth, lHeight);
//        
//       
//            
//            btnCancel = [UIButton buttonWithType:UIButtonTypeRoundedRect];
//
//            btnCancel.titleLabel.font = self.detailsLabelFont;
//            
//            [self.btnCancel addTarget:self action:@selector(modalViewAction:) forControlEvents:UIControlEventTouchUpInside];
//            
//            self.btnCancel.frame=lFrameB;
//            
//            [self.btnCancel setTitle:@"Cancel" forState:UIControlStateNormal];	
//            
//            // Center the text on the button, considering the button's shadow
//            self.btnCancel.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
//            self.btnCancel.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
//            
//            [self.btnCancel roundStyleView];
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == WinkProgressAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == WinkProgressAnimationZoom) {
...
@implementation WinkRoundProgressView
